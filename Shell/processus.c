...
struct fiche_proc {
  ...
  Etat e;
  List_proc suivant;
   char *cmd; /*Commande de lancement du processus*/
};
...
void afficher(List_proc l) {
	if ( l == NULL) {
		...
		while (l != NULL) {
		printf("pid : %d \t pid_reel : %d \t statut : %s \t Commande : %s \n",l -> pid,l ->pid_reel, (l -> e == ACTIF )? "A" : "S",l -> cmd);
		l = l -> suivant;
		}
	}
}
 ...
 * 				- le numéro géré par le shell du nouveau processus (int) D
 *              - l'état du nouveau processus (Etat) (D)
 *              - l'adresse de la liste de processus (List_proc *) (D/R)
 * 				- La ligne de commande lancée (Char *) (D)
...
int inserer_en_tete(int pid, int pidr, char *cmd, Etat e, List_proc *l) {
	List_proc nouveau;
...
/* fonction qui rajoute un processus en fin d'une liste de processus
 * paramètres : - le numéro du nouveau processus (int) (D)
 * 				- le numéro géré par le shell du nouveau processus (int) D
 *              - l'état du nouveau processus (Etat) (D)
 *              - l'adresse de la liste de processus (List_proc *) (D/R)
 ...
 */
 ...
 int inserer_fin(int pid, int pidr,char * cmd, Etat e, List_proc *l){
	 
	List_proc nouveau, pcour;
	/*Pour parcourir et créer le nouveau maillon*/
	
	 if (*l == NULL) {
		 return inserer_en_tete(pid,pidr,cmd,e,l);
	 } else {
  ...
  */
  ...
  /*Fonction qui recherche un processus grace a son pid et retourne son pid réel
  * paramètres : int pid
  * 			 l list proc
  * résultat : pid réel si il existe 0 sinon
  * précondition : liste initialisé 
  * postcondition : /
  */
  
  int realpid ( int pid, List_proc l) {
	    while (( l != NULL) && (l->pid != pid)) {
		 l = l -> suivant;
	 }
	 return l == NULL ? 0 : l -> pid_reel;
}

  
   /*Fonction qui recherche un processus grace a son pid réel et retourne son pid
  * paramètres : int pid
  * 			 l list proc
  * résultat : pid si il existe 0 sinon
  * précondition : liste initialisé 
  * postcondition : /
  */
  
  int shellpid ( int pidreel, List_proc l)	{
	   while (( l != NULL) && (l->pid_reel != pidreel)) {
		 l = l -> suivant;
	 }
	 return l == NULL ? 0 : l -> pid;  
 }
